...
public class KeycloakAdminService {
    ...
    public void assignCustomerRole(String keycloakUserId) {
        try {
            ...
            Map<String, Object> role = getRealmRole(token, "CUSTOMER");
            assignRealmRoleToUser(token, keycloakUserId, role);
            log.info("Assigned CUSTOMER role to user {}", keycloakUserId);
        } catch (Exception e) {
            log.warn("Could not assign CUSTOMER role to user {}: {}", keycloakUserId, e.getMessage());
        }
    }
